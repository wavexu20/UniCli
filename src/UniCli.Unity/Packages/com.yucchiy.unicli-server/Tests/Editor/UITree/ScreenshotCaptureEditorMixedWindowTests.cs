...
{
    [TestFixture]
    public class ScreenshotCaptureEditorMixedWindowTests
    {
        [SetUp]
        public void SetUp()
        {
            MixedCaptureTestEditorWindow.Open();
        }

        [TearDown]
        public void TearDown()
        {
            var windows = Resources.FindObjectsOfTypeAll<MixedCaptureTestEditorWindow>();
        ...
        [UnityTest]
        public IEnumerator Execute_CaptureMixedWindow_IncludesImguiAndUitkContent()
        {
            ...
            var tempDir = Path.Combine(Path.GetTempPath(), "unicli_editor_capture_mixed_tests", Guid.NewGuid().ToString("N"));
            ...
            try
            {
                ...
                {
                    panel = MixedCaptureTestEditorWindow.Title,
                    filename = Path.Combine(tempDir, "mixed.png")
                });
                ...
                try
                {
                    ...
                    Assert.That(ContainsDominantPixel(texture, texture.width / 2, texture.width,
                        color => color.g > 0.50f && color.g > color.r + 0.20f), Is.True);
                }
                finally
                {
